...
sub do_one_dir{
    ...
    print "do_one_dir\n";
    print "do_one_dir from: $src_dir \n";
    print "do_one_dir to: $subtitled_dir \n";
    ...
    foreach(@vfiles){
        ...
        # refitted after using youtube-dl with --write-description, 0409
        $dest = $subtitled_dir . $filename;
        if ( $filename =~ /\.description$/i ){ # *.description file, no encoding
            my $cmd = "mv $full_video_filename $dest ";
            `$cmd`;
        } elsif( $filename =~ /cname/i ){ # might be my cname file
            my $cmd = "mv $full_video_filename $dest ";
            `$cmd`;
        } elsif( $filename =~ /^[.]+$/ ){ # files: . ..
            ;
        } elsif( $filename =~ /^\s*$/ ){ # empty line
            ;
        } elsif (-e $full_video_filename){ # file need to do encoding
            add_subtitle($src_dir, $filename, $subtitled_dir);
            sleep 5;
        }else{;}
        ##sleep 25; ## during testing
...
sub add_subtitle{
    ...
    ## threads=2 !!
    my $pass1cmd = "mencoder  $full_input_name -o /dev/null -oac lavc -ovc lavc -lavcopts autoaspect:threads=2:vpass=1 -sub $subtitle  -utf8 -font 'WenQuanYi Micro Hei' ";
    ...
    my $pass2cmd = "mencoder  $full_input_name -o $full_output_name -oac lavc -ovc lavc -lavcopts vbitrate=15900:autoaspect:threads=2:vpass=2 -sub $subtitle  -utf8 -font 'WenQuanYi Micro Hei' ";
